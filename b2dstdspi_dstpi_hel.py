...
Compound(
  Compound(
    Plane("xy", color = 'r', alpha = 0.3).scale(0.7+0.6*yaxis).shift(-0.6*yaxis+0.2*xaxis), 
    Compound(
      Arrow("x", color = 'k'), 
    ...
    Angle("xy", theta_d, (0.25, 0.28), color = 'k').rotate(zaxis, 180-theta_d), 
    Text(r"$\theta_{D^*}$", 0.5*xaxis, color = 'k').rotate(zaxis, 180-0.4*theta_d), 
  ...
  Line("x", color = "k", linestyle = "--", linewidth = 1).shift(-0.5*xaxis).scale(3.6), 
